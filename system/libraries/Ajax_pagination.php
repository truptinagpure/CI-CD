...
class CI_Ajax_pagination{
    ...
    var $last_link       = 'Last';
    var $uri_segment     = 3;
    var $full_tag_open   = '<div class="pagination" role="navigation" aria-label="pagination">';
    ...
    function create_links(){
        ...
        if ($num_pages == 1){
            $info = '<div class="showing-page">Showing : ' . $this->total_rows .'</div>';
            return $info;
        }
        ...
        if ($this->show_count){
            ...
            $output .= '<div class="of_result">'.$info.'</div>';
        }
        ...
        if  ($this->cur_page != 1){
            ...
            $output .= $this->prev_tag_open 
                    . $this->getAJAXlink( $i, $this->prev_link, true )
                    . $this->prev_tag_close;
        }
        ...
        if ($this->cur_page < $num_pages){
            $output .= $this->next_tag_open 
                . $this->getAJAXlink( $this->cur_page * $this->per_page , $this->next_link, true )
                . $this->next_tag_close;
        }
    ...
    function getAJAXlink($count, $text, $aria_hidden=false) {
        $aria_hidden_attr = '';
        if($aria_hidden)
        {
        ...
        return '<a href="javascript:void(0);"' . $this->anchor_class . ' onclick="'.$this->link_func.'('.$pageCount.')" '.$aria_hidden_attr.'>'. $text .'</a>';
    }
}