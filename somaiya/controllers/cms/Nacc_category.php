...
class Nacc_category extends Somaiya_Controller {
    function __construct()
    {
        parent::__construct('backend');
        $this->load->model('cms/nacc/Nacc_category_model', 'nacc_category');
     ...
     function index()
    {
        ...
        $this->data['child_menu_type']      = "nacc_category";
        $this->data['sub_child_menu_type']  = "nacc_category";
        ...
        $this->data['nacc_list'] = $this->nacc_category->get_nacc_list(['nc.institute_id' => $this->session->userdata('sess_institute_id')]);
        $this->data['title']                = _l("Nacc",$this);
        $this->data['page']                 = "Module";
        $this->data['content']              = $this->load->view('cms/nacc/nacc_category/index',$this->data,true);
    ...
    function edit($id='')
    {
        ...
        $this->data['sub_menu_type']        = "nacc";
        $this->data['child_menu_type']      = "nacc_category";
        $this->data['sub_child_menu_type']  = "save_nacc_category";
        $this->data['post_data']            = [];
        $this->data['id']                   = $id;
        ...
        if($this->form_validation->run($this) === TRUE)
        {
            ...
            {   
                ...
                if($id)
                {
                    ...
                    $response = $this->nacc_category->_update('id', $id, $update, $this->input->post());
                ...
                else
                {
                    ...
                    $response              = $this->nacc_category->_insert($insert,$nacc_files);
                ...
                redirect(base_url().'cms/nacc_category/');
            }
        }

        if($id!='')
        {
            $this->data['links']  = $this->nacc_category->get_all_links($id);

            $nacc     	      = $this->nacc_category->get_nacc_list(['nc.id' => $id]);
            $this->data['post_data']    = isset($nacc[0]) ? $nacc[0] : [];
		    $this->data['all_nacc_list']         = $this->nacc_category->get_all_nacc_list($this->session->userdata('sess_institute_id'),$id);
        ...
        {
            $this->data['all_nacc_list']         = $this->nacc_category->get_all_nacc_list($this->session->userdata('sess_institute_id'),'');
        }

        $this->data['all_accreditation']     = $this->nacc_category->get_all_accreditation($this->session->userdata('sess_institute_id'));
        

        /*$parentCategoryList                  = $this->nacc_category->getParentCategoryByInstitute($this->session->userdata('sess_institute_id'));

        if($parentCategoryList)
        {
            $i = 0;
            foreach ($parentCategoryList as $key => $value) {
                $childCategory = $this->nacc_category->get_subCategoryData($value['id']);
        ...
        $this->data['all_nacc_category']   = $parentCategoryList;*/
        $this->data['content']             = $this->load->view('cms/nacc/nacc_category/form',$this->data,true);
        $this->load->view($this->mainTemplate,$this->data);    
    }
    ...
    function delete($id='')
    {
        ...
        $response = $this->nacc_category->_delete('id', $id);
