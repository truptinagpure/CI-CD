...
class Plant_sub_category extends Somaiya_Controller {
    function __construct()
    {
        parent::__construct('backend');
        $this->load->model('cms/vanaspatyam/Plant_category_model', 'category');
        $this->load->model('cms/vanaspatyam/Plant_sub_category_model', 'sub_category');
    ...
    {   
        ...
        validate_permissions('Plant_sub_category', 'index', $this->config->item('method_for_view'), $this->data['insta_id']);

        $this->data['data_list']            = $this->sub_category->get_sub_categories();
        $this->data['title']                = _l("Module",$this);
        $this->data['page']                 = "Module";
        $this->data['content']              = $this->load->view('cms/vanaspatyam/sub_categories/sub_category',$this->data,true);
    ...
    function edit($id='')
    {
        ...
        validate_permissions('Plant_sub_category', 'edit', $per_action, $this->data['insta_id']);

        if($id!='')
        {
            $this->data['post_data']        = $this->sub_category->get_sub_category_data($id);
            if(empty($this->data['post_data']))
            {
                $this->session->set_flashdata('requeststatus', ['error' => 1, 'message' => 'Plant sub category not found']);
                redirect(base_url()."cms/plant_sub_category/");
            }
        }
        ...
        $this->data['category_list']        = $this->category->get_parent_categories(1);
        ...
        $this->data['content']              = $this->load->view('cms/vanaspatyam/sub_categories/sub_category_form',$this->data,true);
        $this->load->view($this->mainTemplate,$this->data);    
    }
    ...
    {
        $this->data['insta_id'] = $this->default_institute_id;
        validate_permissions('Plant_sub_category', 'delete', $this->config->item('method_for_delete'), $this->data['insta_id']);
