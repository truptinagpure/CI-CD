...
if ( ! function_exists('_l')){
    function _l($label, $obj)
    {
        ...
        if ($return){
            ...
            if($obj->router->fetch_class()=="Somaiya_general"){
                if(!in_array($label,$obj->langArray)){
                    $file = getcwd()."/somaiya/language/".$_SESSION["language"]["language_name"].'/'.$_SESSION["language"]["code"]."_lang.php";
                    if(file_exists($file)){
            ...
            }elseif($obj->router->fetch_class()=="Somaiya_general_admin"){
                if(!in_array($label,$obj->langArray)){
                    $file = getcwd()."/somaiya/language/".$_SESSION["language"]["language_name"]."/backend_lang.php";
                    if(file_exists($file)){
                        $current = file_get_contents($file);
                        $current .= "\n";
                        $current .= '$lang["'.$label.'"] = "'.$label.'";';
...
if ( ! function_exists('frontendStatisticCalc')){
    function frontendStatisticCalc($obj,$language){
        //        Statistic
        $visitorMinDate = $obj->Somaiya_general_model->getMinDateVisitor();
        if($visitorMinDate!=0){
            $visitorMaxDate = $obj->Somaiya_general_model->getMaxDateVisitor();
            ...
            if($times >= 86400){
                ...
                for($d=0;$d<$days;$d++){
                    ...
                    $obj->Somaiya_general_model->updateStatistic(strtotime(date("d.m.Y",$visitorMinDate)),$maxTime);
                    $visitorMinDate = $maxTime;
                }
            }
        }
        $visitor = $obj->Somaiya_general_model->getDuplicateVisitor(session_id(),$_SERVER["REQUEST_URI"]);
        if(count($visitor) == 0){
            // Get IP address
            ...
            $obj->Somaiya_general_model->insertVisitors($visitor_data);
        }else{
            ...
            $obj->Somaiya_general_model->updateDuplicateVisitor(session_id(),$_SERVER["REQUEST_URI"],$visitor_data);
        }
    }
}