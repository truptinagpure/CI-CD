...
if ( ! function_exists('myfull_date')){
...
if ( ! function_exists('myfull_int_date')){
...
function curl_get_data($curl_data)
{
    $ch             = curl_init();
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
    curl_setopt($ch, CURLOPT_FOLLOWLOCATION, TRUE);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
    curl_setopt($ch, CURLOPT_URL, $curl_data['url']);
    curl_setopt($ch, CURLOPT_TIMEOUT, 80);
    $result         = curl_exec($ch);
    if(curl_error($ch))
    {
        $response   = ['error' => 1, 'message' => 'Curl call error: '.curl_error($ch)];
    }
    else
    {
        if(!empty($result))
        {
            $response   = ['error' => 0, 'message' => 'Success', 'result' => json_decode($result)];
        }
        else
        {
            $response   = ['error' => 2, 'message' => 'Failure'];
        }
    }
    curl_close($ch);
    return $response;
...
function curl_post_data($curl_data)
{
    $ch             = curl_init($curl_data['url']);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLINFO_HEADER_OUT, true);
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_POSTFIELDS, $curl_data['post_data']);
    curl_setopt($ch, CURLOPT_HTTPHEADER, $curl_data['header']);
    $result         = curl_exec($ch);
    curl_close($ch);
    
    if($result === FALSE)
    {
        // die('Oops! Curl Error: ' . curl_error($ch));
        $response   = ['error' => 1, 'message' => 'Curl call error'];
    }
    else
    {
        $response   = ['error' => 0, 'message' => 'Success', 'result' => json_decode($result)];
...
function handle_number_format($number, $decimal_places=2)
{
