...
class Nacc_childcategory_model extends CI_Model
{
    ...
    function get_nacc_list($conditions=[])
    {
		...
		$this->db->select('nc.*, eid.INST_NAME, ac.name as accreditation_name, sj.name as subcategory_name, sjc.name as category_name');
        ...
        $this->db->join('nacc sj', 'nc.parent_id = sj.id', 'left');
        $this->db->join('nacc sjc', 'sj.parent_id = sjc.id', 'left');
        ...
        $this->db->where('nc.parent_id NOT IN (SELECT id FROM nacc where parent_id = 0)', NULL, FALSE); 
    ...
    function get_all_nacc_list($institute_id,$accreditation_id)
    {
        $this->db->select("id, name");
        $this->db->from('nacc');
        $this->db->where('institute_id',$institute_id);
        if(isset($accreditation_id) && !empty($accreditation_id))
        {
            $this->db->where('accreditation_id =',$accreditation_id);
        }
    ...
    function get_all_subcategory_list($institute_id,$category_id)
    {
        $this->db->select("id, name");
        $this->db->from('nacc');
        $this->db->where('institute_id',$institute_id);
        ...
        //$this->db->where('parent_id',0);
        //$this->db->where('parent_id IN (SELECT id FROM nacc where parent_id = 0)', NULL, FALSE); 
        $this->db->where('status',1);
        $this->db->order_by('name', 'ASC');
        $query = $this->db->get();
        return $query->result_array();
    ...
    function get_categoryId_by_subcategory($subcategory_id)
    {
        $this->db->select('nc.parent_id');
        $this->db->from('nacc nc');
        $this->db->where('nc.id',$subcategory_id);
        $this->db->where('nc.status', 1);
    ...
    function get_subcategory_by_cat_id($category_id)
    {
        ...
        if(count($category_id) >= 2)
        {
            ...
            $this->db->where_in('nc.parent_id', $category); 
        }
        else
        {
            $parent_id = $category_id;
            $this->db->where('nc.parent_id', $parent_id);
            //$this->db->where('find_in_set("'.$parent_id.'", nc.parent_id) <> 0'); 

        }

        $this->db->where('nc.status =', '1');
        $this->db->order_by('nc.id', 'ASC');
        $query = $this->db->get();
        return $query->result_array();
    }

    function get_category_by_accreditation_id($accreditation_id)
    {
        $this->db->select("nc.id, nc.name");
        $this->db->from('nacc nc');

        if(count($accreditation_id) >= 2)
        {
            $accreditation = implode(',', $accreditation_id);
            $this->db->where_in('nc.accreditation_id', $accreditation); 
        }
        else
        {
            //$parent_id = $accreditation_id;
            $this->db->where('nc.accreditation_id', $accreditation_id);
            //$this->db->where('find_in_set("'.$parent_id.'", nc.parent_id) <> 0'); 
