...
class Research_support_team_model extends CI_Model
{
    ...
    {
        $this->db->select('rst.*, concat(emd.FIRST_NAME, " ", emd.LAST_NAME) as name');
        $this->db->from('research_support_team rst');
        $this->db->join('edu_member_dir emd','emd.MEMBER_ID = rst.researcher_member_id');
        ...
        $this->db->where('rst.public !=', '-1');
        $this->db->order_by('rst.id', 'DESC');
        $query = $this->db->get();
    ...
    {
        $this->db->select('*');
        $this->db->from('research_support_team');
