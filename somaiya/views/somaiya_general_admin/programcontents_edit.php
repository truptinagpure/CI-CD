    ...
    <!-- BEGIN SAMPLE FORM PORTLET-->
        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                ...
                &nbsp;&nbsp;<span class="custpurple"><a class="brownsmall btn brown" href="<?php echo base_url('admin/programcontents'.(isset($data_type)?"/".$data_type:'').(isset($relation_id)?"/".$relation_id:"")); ?>">Back </a></span>
            </div>
            <div class="portlet-body form" id="drag">
                    ...
                    <form id="user_manipulate" class="cmxform form-horizontal tasi-form" method="post" action="<?=$base_url.$page."_manipulate".(isset($data['contents_id'])?"/".$data['contents_id']:"")?>">
                        ...
                        <?php 
                            if(isset($data['contents_id']))
                            {
                                mk_htext("title",_l('Programme Name',$this),isset($data['title'])?$data['title']:'','readonly');
                            }
                            else
                            {
                                foreach ($program_name as $key => $value) {
                                    mk_htext("title",_l('Programme Name',$this),isset($value['title'])?$value['title']:'','readonly');
                                } 
                            }
                        ?>
                        ...
                        ?>
                            <div class="fields_wrap">
                                <div class="form-group ">
                                    <label for="url" class="control-label col-lg-2">Apply Now</label>
                                    <div class="col-lg-10">
                                        <input class=" form-control" id="enable_apply_now" name="enable_apply_now" value="<?php echo isset($data['enable_apply_now'])?$data['enable_apply_now']:''; ?>" type="text">
                                    </div>
                                </div>
                            </div>
                        <?php //}  else { ?>
                            <!-- <input class=" form-control" id="enable_apply_now" name="enable_apply_now" value="<?php echo isset($data['enable_apply_now'])?$data['enable_apply_now']:''; ?>" type="hidden"> -->
                        <?php //} ?>
                        ...
                        ?>
                            <div class="fields_wrap">
                                <div class="form-group ">
                                    <label for="enable_enquire_now" class="control-label col-lg-2">Enquire Now</label>
                                    <div class="col-lg-10">
                                        <input class=" form-control" id="enable_enquire_now" name="enable_enquire_now" value="<?php echo isset($data['enable_enquire_now'])?$data['enable_enquire_now']:''; ?>" type="text">
                                    </div>
                                </div>
                            </div>
                        <?php //}  else { ?>
                            <!-- <input class=" form-control" id="enable_enquire_now" name="enable_enquire_now" value="<?php echo isset($data['enable_enquire_now'])?$data['enable_enquire_now']:''; ?>" type="hidden"> -->
                        <?php //} ?>

                        <div class="fields_wrap">
                            <div class="form-group ">
                                <label for="enable_admission" class="control-label col-lg-2">Admission Details</label>
                                <div class="col-lg-10">
                                    <input class=" form-control" id="enable_admission" name="enable_admission" value="<?php echo isset($data['enable_admission'])?$data['enable_admission']:''; ?>" type="text">
                                </div>
                            </div>
                        </div>
                        ...
                        ?>
                            <div class="form-group">
                                <label for="display_on_stage" class="control-label col-lg-2">Display on Stage Only</label>
                                <div class="col-lg-10 col-sm-10">
                                    <input style="width: 20px" class="checkbox form-control" id="display_on_stage_checkbox" type="checkbox" <?php if(isset($data['display_on_stage']) && $data['display_on_stage'] == 1){ echo 'checked="checked"'; } ?>>
                                    <input value="<?php echo set_value('display_on_stage', (isset($data['display_on_stage']) ? $data['display_on_stage'] : '')); ?>" style="display: none;" id="display_on_stage" name="display_on_stage" checked="" type="text">
                                </div>
                            </div>
                        <?php } ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function() 
    {
        ...
        if ($('#display_on_stage_checkbox').is(':checked')) {
            $('#display_on_stage').val(1);
        }else{
            $('#display_on_stage').val(0);
        }

        $('#display_on_stage_checkbox').click(function() {
            if($(this).is(':checked')){
                $('#display_on_stage').val(1);
            }else{
                $('#display_on_stage').val(0);
...
$.validator.addMethod("check_unique_url", function(value) {
        ...
        $.ajax({
            type: "POST",
            url: base_url+"admin/ajax_check_program_url",
            async: false,
            data: {url : url, contents_id : contents_id, relation_id : relation_id},
            success: function(response){
