<div class="form-group ">
    <label for="status" class="control-label col-lg-2">Donation Documents</label>
    <div class="col-lg-10 col-sm-10">
        <div class="boxform">
            <table style="width: 100%;">
                <tbody id="page_list">
            <?php if(isset($donation_documents) && count($donation_documents)!=0){ ?>
                <input type="hidden" name="links_array_check" value="2">
                <?php
                    $in = 0;
                    foreach ($donation_documents as $key5 => $data5) { 
                ?>
                        <tr class="group_wrap all-scroll" data-id="<?php echo $data5['id'] ?>">
                            <td class="link_name">
                                <div class="p10">
                                    <input type="text" id="linkname<?php echo $key5; ?>" name="link_name[]" value="<?php if(isset($data5['link_name'])) { echo $data5['link_name']; } ?>" Placeholder="Document Name">
                                </div>
                            </td>
                            <td class="link_url">
                                <div class="p10">
                                    <input type="text" id="linkurl<?php echo $key5; ?>" name="link_url[]" value="<?php if(isset($data5['link_url'])) { echo $data5['link_url']; } ?>" Placeholder="Document URL">
                                </div>
                            </td>
                ...
                <?php $in++;}} else { ?>
                    ...
                    <tr class="group_wrap">
                        <td class="link_name">
                            <div class="p10">
                                <input type="text" id="linkname0" name="link_name[]" Placeholder="Document Name">
                            </div>
                        </td>
                        <td class="link_url">
                            <div class="p10">
                                <input type="text" id="linkurl0" name="link_url[]" Placeholder="Document URL">
                            </div>
                        </td>
                    </tr>
                    <div style="padding-bottom:20px;">
...
$(document).on('click', 'a.add', function (e) {
    e.preventDefault();
    $(".group_wrap:last").after('<tr class="group_wrap"><td class="link_name"><div class="p10"><input type="text" id="linkname'+i+'" name="link_name[]" Placeholder="Document Name"></div></td><td class="link_url"><div class="p10"><input type="text" id="linkurl'+i+'" name="link_url[]" Placeholder="Document URL"></div></td><td class="add_remove_td"><div class="p10"><a class="removeMore'+i+'" data-id=""><i title="<?=_l("Delete",$this)?>" class="fa fa-trash-o"></i></a></div></td></tr>');
    // removeMore();
    $(".removeMore"+i).click(function(){
        ...
        if(answer)
        {
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('cms/donation/delete_donation_documents');?>",
                data: "pid="+pid,
    ...
    $(document).ready(function()
    {
        $( "#page_list" ).sortable({
            ...
            update  : function(event, ui)
            {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo site_url('cms/donation/save_donation_documents_order');?>",
