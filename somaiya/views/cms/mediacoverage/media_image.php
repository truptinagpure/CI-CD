...
$media_image_url         = $media_default_image_url = base_url()."upload_file/images20/default.png";
$delete_image            = false;
    if(isset($mediacoverage_data['image']) && !empty($mediacoverage_data['image']))
    {
        $delete_image       = true;
        $media_image_url    = base_url()."upload_file/images20/".$mediacoverage_data['image'];
    }

    $mediacoverage_id           = isset($mediacoverage_data['mediacoverage_id']) ? $mediacoverage_data['mediacoverage_id'] : '';
    //$contents_id        = isset($mediacoverage_data['contents_id']) ? $mediacoverage_data['contents_id'] : '';
    ...
    <div class="col-lg-10 col-sm-10">
        <div class="row"> 
            <div class="col-md-12">
                ...
                <?php if(isset($mediacoverage_data['image']) && !empty($mediacoverage_data['image'])){ ?>
                    <img id="croppie-image" class="croppie-img" src="<?php echo $media_image_url; ?>" height="170px" width="170px" />
        ...
        <div class="col-md-1">
            <?php if($delete_image){ ?>
                <a class="btn btn-sm btn-danger" id="delete_image" href="javascript:void(0);" title="Delete" onclick="delete_image('<?php echo $mediacoverage_id; ?>');"><i class="icon-trash"></i></a>
            <?php } ?>
        </div>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-12 text-center">
                                <div id="media_image_demo"  style=""></div>
                            </div>
...
<script type="text/javascript">
	...
	function delete_image(mediacoverage_id) {
		var media_default_image_url = '<?php echo $media_default_image_url; ?>';
        var answer = confirm ("Are you sure you want to delete from this image?");
        if (answer)
        {
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('cms/mediacoverage/deleteimage');?>",
                data: "mediacoverage_id="+mediacoverage_id,
                success: function (response) {
                    if(response == 1)
                    {
                    	...
                    	$('#media_image_src').attr('src', media_default_image_url);
...
<script type="text/javascript">
    $(document).ready(function() {
        $image_crop = $('#media_image_demo').croppie({
            enableExif: true,
            viewport: {
                width: 200,
                height: 200,
