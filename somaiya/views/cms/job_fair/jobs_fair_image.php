...
$jobs_fair_image_url         = $jobs_fair_default_image_url = base_url()."upload_file/kjsieit/jobs_fair/images/default.png";
$delete_image                = false;
    if(isset($post_data['image']) && !empty($post_data['image']))
    {
        $delete_image        = true;
        $jobs_fair_image_url = base_url()."upload_file/kjsieit/jobs_fair/images/".$post_data['image'];
    }

    $jobs_fair_id            = isset($post_data['id']) ? $post_data['id'] : '';
?>
...
<div class="form-group">
    ...
    <div class="col-lg-10 col-sm-10">
        <div class="row"> 
            <div class="col-md-12">
                ...
                <?php if(isset($post_data['image']) && !empty($post_data['image'])){ ?>
                    <img id="croppie-image" class="croppie-img" src="<?php echo $jobs_fair_image_url; ?>" height="170px" width="170px" />
        ...
        <div class="col-md-1">
            <?php if($delete_image){ ?>
                <a class="btn btn-sm btn-danger" id="delete_image" href="javascript:void(0);" title="Delete" onclick="delete_image('<?php echo $jobs_fair_id; ?>');"><i class="icon-trash"></i></a>
            <?php } ?>
        </div>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-12 text-center">
                                <div id="jobs_fair_image_demo"  style=""></div>
                            </div>
...
<script type="text/javascript">
	...
	function delete_image(id) {
		var jobs_fair_default_image_url = '<?php echo $jobs_fair_default_image_url; ?>';
        var answer = confirm ("Are you sure you want to delete from this image?");
        if (answer)
        {
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('cms/job_fair/deleteimage');?>",
                data: "id="+id,
                success: function (response) {
                    if(response == 1)
                    {
                    	...
                    	$('#jobs_fair_image_src').attr('src', jobs_fair_default_image_url);
...
<script type="text/javascript">
    $(document).ready(function() {
        $image_crop = $('#jobs_fair_image_demo').croppie({
            enableExif: true,
            viewport: {
                width: 621,
                height: 352,
