...
$industry_image_url        = $industry_default_image_url = base_url()."upload_file/kjsieit/industrial_visits/images/default.png";
$delete_image           = false;
    if(isset($post_data['image']) && !empty($post_data['image']))
    {
        $delete_image       = true;
        $industry_image_url = base_url()."upload_file/kjsieit/industrial_visits/images/".$post_data['image'];
    }

    $industry_id               = isset($post_data['id']) ? $post_data['id'] : '';
?>

<!-- <div class="row">
    <div class="col-md-3">
        <div class="form-group">
            <label for="industry_image">File input</label>
            <input type="file" id="image" name="image">
        </div>
    </div>
    <div class="col-md-8">
        <img id="industry_image_src" src="<?php echo $industry_image_url; ?>" width="20%" height="20%">
    </div>
    <div class="col-md-1">
        <?php if($delete_image){ ?>
            <a class="btn btn-sm btn-danger" id="delete_image" href="javascript:void(0);" title="Delete" onclick="delete_image('<?php echo $industry_id; ?>');"><i class="icon-trash"></i></a>
        <?php } ?>
    </div>
</div> -->
...
<div class="form-group">
    <label class="control-label col-lg-2">Thumbnail Image&nbsp;<span><a title="" data-toggle="tooltip" data-placement="right" data-original-title="Upload image dimensions are 621 pixels (width) x 352 pixels (height). And size of file should be less than 2mb."><i class="fa fa-info-circle" aria-hidden="true"></i></a></span></label>
        ...
        <div class="row"> 
            <div class="col-md-12">
                ...
                <?php if(isset($post_data['image']) && !empty($post_data['image'])){ ?>
                    <img id="croppie-image" class="croppie-img" src="<?php echo $industry_image_url; ?>" height="170px" width="170px" />
                ...
                <!-- <div class="image_error error_msg"><?php //echo form_error('image', '<label class="error">', '</label>'); ?></div> -->
            </div>
        </div>
...
<script type="text/javascript">
	...
	function delete_image(id) {
		var industry_default_image_url = '<?php echo $industry_default_image_url; ?>';
        var answer = confirm ("Are you sure you want to delete from this image?");
        if (answer)
        {
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('cms/industrial_visits/deleteimage');?>",
                data: "id="+id,
                success: function (response) {
                    if(response == 1)
                    {
                    	...
                    	$('#industry_image_src').attr('src', industry_default_image_url);
