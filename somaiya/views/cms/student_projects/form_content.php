            ...
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <?php if(isset($student_project_content_id) && !empty($student_project_content_id)) { ?>
                            <span class="caption-subject font-brown bold uppercase">Edit Student Project Content</span>
                        <?php } else { ?>
                             <span class="caption-subject font-brown bold uppercase">Add New Student Project Content</span>
                       <?php } ?>
                    </div>      
                    &nbsp;&nbsp;<span class="custpurple"><a class="brownsmall btn brown" href="<?php echo base_url('projects/project_content/'.$student_project_id); ?>">Back </a></span>
                </div>
                <div class="portlet-body form">
                    <div class="form-body">
                        <form id="manage_form" class="cmxform form-horizontal tasi-form" method="post" action="">
                           <input type="hidden" name="project_content_id" value="<?php echo $student_project_content_id; ?>">
                            <input type="hidden" name="project_id" value="<?php echo $student_project_id; ?>">
                            ...
                            <div class="form-group">
                                <div class="col-lg-offset-2 col-lg-10">
                                    ...
                                    <a href="<?php echo base_url('cms/studentprojects/project_content/'.$student_project_id) ?>" class="btn btn-default" type="button">Cancel</a>
                                </div>
                            </div>
                        </form>
                    </div>
    ...
    <script type="text/javascript">
        ...
        $("#manage_form").validate({
            ...
            submitHandler: function(form){
                var project_content_id   = '<?php echo $student_project_content_id; ?>';
                var project_id                  = '<?php echo $student_project_id; ?>';
                var language_id                 = $('#language_id').val();
                var check_result                = true;
                $('.languageerror').html('');

                function check_language(project_content_id, language_id) {
                    var check_language_result   = true;
                    $.ajax({
                        type: "POST",
                        url: base_url+"projects/ajax_check_language",
                        async: false,
                        data: {project_content_id : project_content_id, project_id : project_id, language_id : language_id},
                ...
                check_result = check_language(project_content_id, language_id);

                if(check_result == false)
                {
                    $('.languageerror').html('Project content for this language is already exist.');
                    return false;
                }
                else
                {
