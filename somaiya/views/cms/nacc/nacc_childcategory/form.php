            ...
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <?php if(isset($post_data['id'])) {      ?>
                        <span class="caption-subject font-brown bold uppercase">Edit Child category</span>
                    <?php } else { ?>
                         <span class="caption-subject font-brown bold uppercase">Add New Child category</span>
                   <?php } ?>    
                </div>  
                &nbsp;&nbsp;<span class="custpurple"><a class="brownsmall btn brown" href="<?php echo base_url('cms/nacc_childcategory/'); ?>">Back </a></span>
            </div>
                    ...
                    <form id="nacc_form" class="cmxform form-horizontal tasi-form" enctype='multipart/form-data' method="post" action="">
						...
						<div class="form-group ">
                            ...
                            <div class="col-lg-10">
                                ...
                                <select id="accreditation_id" class="form-control input-lg select2-multiple" name="accreditation_id[]" data-placeholder="Please select accreditation" required>
                        ...
                        ?>
                         <div class="form-group ">
                            ...
                            <div class="col-lg-10">
                                ...
                                <select id="category_id" class="form-control input-lg select2-multiple" name="category_id[]" data-placeholder="Please select category" required>
                                    <option value="">-- Please select category --</option>
                                    <?php               
                                        foreach ($all_nacc_list as $key => $value) {
                                        ?>
                                           <option value="<?php echo $value['id']; ?>" <?php if(isset($category_id) && in_array($value['id'], $category_id)) { echo 'selected="selected"'; } ?>><?php echo $value['name']; ?></option>
                                        <?php
                        ...
                        <div class="form-group ">
                            <label for="parent_id" class="control-label col-lg-2">Subcategory <span class="asterisk">*</span></label>
                            <div class="col-lg-10">

                                <select id="sub_category_id" class="form-control input-lg select2-multiple" name="parent_id[]" data-placeholder="Please select subcategory" required>
                                    <option value="">-- Please select subcategory --</option>
                                    <?php               
                                        foreach ($all_subcategory_list as $key => $value) {
                                        ?>
                        ...
                        <?php $this->load->view('cms/nacc/nacc_childcategory/nacc_files'); ?>
                        ...
                        <div class="form-group">
                            <div class="col-lg-offset-2 col-lg-10">
                                ...
                                <a href="<?php echo base_url('cms/nacc_childcategory/') ?>" class="btn btn-default" type="button">Cancel</a>
                            </div>
                        </div>
                    </form>
                </div>
...
<script type="text/javascript">
    ...
    $(document).on('change','#category_id',function(e){
        console.log("onchange category_id called");
        var selected_category_id=[];
        $. each($('select#category_id option:selected'), function(){
        ...
        //$(".loader").show();
        $.ajax({
            url : "<?php echo base_url(); ?>cms/nacc_childcategory/get_subcategory_by_cat_id",
            type: "POST",
            ...
            success: function(data, textStatus, jqXHR)
            {
                ...
                $("#sub_category_id").html(categorylisthtml);
                //$(".loader").hide();
            },
    ...
    // get result category base on accrediation id

    $(document).on('change','#accreditation_id',function(e){
        console.log("onchange accreditation_id called");
		//$("#sub_category_id option[value='']").attr('selected', true)
        $('#sub_category_id').empty(); // clear selected subcategory
		
        var selected_accreditation_id=[];
        $. each($('select#accreditation_id option:selected'), function(){
            selected_accreditation_id. push($(this).val());
        });
        
        
        //$(".loader").show();
        $.ajax({
            url : "<?php echo base_url(); ?>cms/nacc_childcategory/get_category_by_accreditation_id",
            type: "POST",
            data : 'accreditation_id='+selected_accreditation_id,
            success: function(data, textStatus, jqXHR)
            {
                //console.log(data);
                var obj = $.parseJSON(data);
                ...
                $("#category_id").html(categorylisthtml);
                //$(".loader").hide();
            },
            error: function (jqXHR, textStatus, errorThrown)
            {
